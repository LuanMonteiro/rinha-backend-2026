...
async function main() {
  ...
  const dimBins = new Map<number, number>([[6, 16], [5, 16], [2, 4], [0, 4], [1, 4]]);
  ...
  console.log("=== Microbenchmark: Extreme Pruning ===");
  console.log(`Config: ${JSON.stringify(Array.from(dimBins.entries()))}`);
  ...
  console.log(`p95: ${(p95 * 1000).toFixed(1)}μs`);
